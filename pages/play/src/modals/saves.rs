...
fn Controls(
    ...
    let is_disconnected = move || status.get() != ConnectionStatus::Disconnected;

    view! {
        <input node_ref=input type="file" style:display="none" on:change=on_file />
        <div class=style::controls>
            <button on:click=on_upload prop:disabled=is_disconnected>
            ...
            <button on:click=on_delete prop:disabled=is_disconnected>
                <svg viewBox="0 0 18 18">
